...
bool AccountManager::changePassword(endstone::Player& pl, const std::string& old_password, const std::string& new_password) {
    ...
    if (!validatePassword(trimmedNewPassword)) {
        pl.sendMessage(endstone::ColorFormat::Red + "Новый пароль должен быть не менее 4 символов!");
        return false;
    }
    ...
    if (currentData.password != hashedOldPassword) {
        pl.sendMessage(endstone::ColorFormat::Red + "Неверный старый пароль!");
        return false;
    }
    ...
    pl.sendMessage(endstone::ColorFormat::Green + "Пароль успешно изменен!");
    return true;
}
